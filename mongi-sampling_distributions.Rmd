...
Answer - From the summary of the area vector, it is clear that the IQR is 1743 MINUS 1126, that means 50% of the sample is within this range (1126,1743). To show it clearly, we are drawing here the box plot for this vector

```{r}
boxplot(area)
```

Also, seeing the histogram above, it is clear that the distribution is right skewed.
...
Here the samp1 will always be different as the random sample we took will vary every time we knit this document. For example for the first time I knit this document, the distribution came out to be similar to the overall population distribution but less of right skew. It will vary in this knit document as I knit the document many times before it was actually published in RPubs.
...
The sample of size 1000 will provide the most accurate estimate of the population mean. As the sample size increases, the estimate becomes better as the random high or lows get balanced out as the sample size becomes more.
...
Answer - There are 5000 elements in sample_means50
```{r}
sprintf("Number of elements in sample_means50 is: %s", length(sample_means50))
```
As the sample would change every time we knit the document, that is why it is better to give the value of the center and the distribution of the sample programatically.
...
Yes, the the distribution would be more normal if we collected 50,000 sample means. The more the sample size, the better estimate it gived for the population. Shown below with the code:
```{r}
...
sprintf("Number of elements in sample_means_small is : %s", length(sample_means_small))
```

The new vector sample_means_small is a vector of 100 elements which has the means of 100 samples of 50 elements taken randomly. This is to show how the means would would be for any random 100 such samples.
...
Answer - As the sample size increases, the means start to lie more nearer to the actual population mean. Smaller the sample size, more spread will be the means sampling distribution. Hence we see here that as the sample size moves from 10 to 50 to 100 - keeping the number of samples same for all the 3 sizes, the mean distribution shrinks and becomes very close to the center, that means, the mean becomes more closer the actual population mean.
...
```{r}
samp1_price <- sample(price, 50)
mean(samp1_price)
```

Note here that every time we knit the document, a new sample of size 50 gets picked from the price vector, and hence the mean would vary every time.
...
So, from the above plot we see that the distribution is skewed on the right. But here the normal distribution can be used as:
1) The sample observations are independent, the sample size if less than 10% of the population which is 2930.
2) The sample size is large, which is 50. Generally if n >= 30, the size is considered large
3) The sample distribution is skewed to the right but as the sample size is large, we can safely say the normal distribution can be used.

```{r}
sprintf("The mean of the population can be guessed to be the mean of the sample distribution which is : %s", mean(sample_means50))
```

Comparing the mean of the sample to the actual mean of the population.
```{r}
sprintf("The actual population mean for prices is: %s", mean(price))
```

Hence we can see, the 2 values are so close.
...
```{r}
sample_means150 <- rep(price, 5000)
