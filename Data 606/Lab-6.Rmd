...
These are sample statistics as this is the per centage of people from the overall people who took this poll.
...
Answer : To generalize the report, we assume that this is random sampling. Random sampling is a good assumption for this, as the samples taken were independent.
...
Each row in table 6 corresponds to the sample statistics for each country that was a part of the survey.
Each row in atheism table (table 4) shown on page 13, shows the per centage of sample in each country who selected atheism as their response for both the sruveys - one taken in 2005 and other one in 2012.
...
From above, the percentage of atheists from the data set comes out to be 4.99% which is very close to 5% as given in the table 6.
...
Answer : For the sample surveyed, 13% of the sample voted as atheists.
...
Answer : There are 3 conditions which need to be met for inference:
1) The sample observations are independent. As the overall population on the earth is very huge, sample is much less than 10% of the overall population. Hence we can say that the sample observations are independent.
2) The sample size = 51927, which is > 30. Hence the sample size is large.
3) As the overall count is 51927 which is much more than 100, we can safely say that the normal distribution applies.

Hence all the 3 conditions for inference are met.
...
```{r}
SE_6 <- 0.0069
Z_critical_6 <- qnorm(0.0499, mean = 0, sd = 1, lower.tail = TRUE)
ME_6 <- Z_critical_6 * SE_6
ME_6
```
...
inference(india12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
SE_india12 <- 0.0052
p_india12_atheist <- 0.0302
Z_critical_india12 <- qnorm(p_india12_atheist, mean = 0, sd = 1, lower.tail = TRUE)
ME_india12 <- Z_critical_india12 * SE_india12

ME_india12
```

All 3 conditions for inference met for India sample set:
1) The sample observations are independent. As the overall population on the earth is very huge, sample is much less than 10% of the overall population. Hence we can say that the sample observations are independent.
2) The sample size = 1092 for India, which is > 30. Hence the sample size is large.
3) As the overall count is 1092 which is much more than 100, we can safely say that the normal distribution applies.
...
inference(germany12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
SE_germany12 <- 0.0052
p_germany12_atheist <- 0.0302
Z_critical_germany12 <- qnorm(p_germany12_atheist, mean = 0, sd = 1, lower.tail = TRUE)
ME_germany12 <- Z_critical_germany12 * SE_germany12

ME_germany12
```

All 3 conditions for inference are met for Germany as well just like India above.
...
Answer: The margin of error is 0 when the population proportion is at the 2 extreme ends, and it continuously increases as we move from the lowest proportion extreme to the center (0.5 proportion) or as we move from the highest proportion to the center (0.5 proportion)
