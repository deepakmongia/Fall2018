...
From the above plots, it looks clear that the male heights is a clean normal ditribution while female heights is not a very clean normal distribution, but it is still quite close to a normal distributuion.
...
As seen above from the plot, the points tend to follow / lie on the line but there are some errant points towards the right tail. But it will be different every time RPubs is generated, because it will provide new values as per a new run of rnorm function.
...
Answer : Yes, the simulated plot looks very similar to the normal probability plot.
Yes, if we try the same above command to draw the simulated plots again and again, we see that female heights are nearly normal.
...
From the normal probability plot for female weight, the observations look to be quite off the line. Hence we cannot suggest that the weight follow a normal distribution.
...
Answer - Let us take the height value of 170, that means q = 170
```{r}
# Using the pnorm first 
1 - pnorm(q = 170, mean = fhgtmean, sd = fhgtsd)

# using the empirical way
sum(fdims$hgt > 170) / length(fdims$hgt)
```

From above, we see that the pnorm gives the value 0.217, and the empircal way gives the value 0.223, which are not reasonable close. As mentioned above for the height of 182, the more close the distribution is towards normal, the more accurate these 2 probability results will be. 

Now let us check for the weights:
```{r}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

# Taking the observation q = 70 for weight
# Probability using the pnorm function
1 - pnorm(q = 70, mean = fwgtmean, sd = fwgtsd)

# Probability using the empirical formula
sum(fdims$wgt > 70) / length(fdims$wgt)

```


We see above, the weight probability of 70 is 0.157 using the empirical way, and 0.164 using the pnorm function. Both the values are reasonably close, and hence we can say the distribution is closer to normal.
If we see the difference here, it is 0.007 while the same for heights showed .006. Both are very close, so we cannot exaclty say which of the 2 observations - height or weight - follow the normal distribution. But if we have to select, we should say that height is more closed to normal distributuion.
...
Answers - 
female bii.di   -->       Q-Q Plot B
female elb.di   -->       Q-Q Plot C
general age     -->       Q-Q Plot D
female che.de   -->       Q-Q Plot A
...
From the above plot, it can be said that the female knee diameter data set is strongly skewed to the right. 

Drawing the fistogram for the same to prove the above point.
```{r}
hist(fdims$kne.di)
```
